...
function OptionCard({
  ...
  if (!verdict.approved) {
    return (
      <div className="rounded-md border-2 border-burn bg-burn/10 p-2.5">
        <p className="font-narrow text-xs font-semibold uppercase tracking-widest text-burn">Rejected</p>
        <h3 className="mt-0.5 font-body text-sm font-semibold text-paper">{option.description}</h3>
  ...
  return (
    <div className="rounded-md bg-paper p-2 text-ink">
      <p className="font-narrow text-xs uppercase tracking-widest text-ink/50">Approved</p>
      <h3 className="mt-0.5 font-body text-sm font-medium text-ink">{option.description}</h3>
      <p className="mt-0.5 font-body text-xs text-ink/60">Recovers roughly {option.minutes_recovered} minutes.</p>
      <button
        type="button"
        onClick={onApprove}
        disabled={approved}
        className="mt-1 rounded-sm bg-ink px-4 py-1 font-narrow text-xs font-semibold uppercase tracking-wide text-paper transition-transform duration-150 enabled:active:scale-95 disabled:opacity-60"
      >
...
export function RecoveryOptions({ recovery }: { recovery: RecoverySnapshot }) {
  ...
  return (
    <section aria-label="Recovery options" className="flex flex-col gap-0.5 px-4 py-0.5">
